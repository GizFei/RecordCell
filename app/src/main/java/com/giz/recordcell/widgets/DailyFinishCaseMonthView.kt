...
class DailyFinishCaseMonthView(context: Context) : BaseCustomMonthView(context) {
    ...
    override fun onDrawSelected(
        ...
        return if(hasScheme){
            ...
            canvas?.drawRoundRect(x.toFloat(), y.toFloat() + 4f, x + mItemWidth.toFloat(),
                y + mItemHeight.toFloat() - 4f, 16f, 16f, selectedBgPaint)
            true
        }else{
            changeSelectedTextColor = true
            selectedBgPaint.style = Paint.Style.FILL
            super.onDrawSelected(canvas, calendar, x, y, hasScheme)
        }
    }
